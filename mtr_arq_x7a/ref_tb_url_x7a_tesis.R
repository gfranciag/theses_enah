...
# de 1946 a 2022
# Consulta a la B. G. Bonfil Batalla de la ENAH con la clasficacion X7A maestría
# Usa un XLSX con el listado de 8 consultas y extraer las URL de 154 tesis
# 
# El resultado se usa para luego extraer toda la metadata de cada 154 
...
url <- readxl::read_excel("D:/_2014-2022/2023/Redes_tesis/mtr_arq_x7a/url20tesisxhoja.xlsx")
...
for(i in 1:length(url$url)){
  ...
  consulta_x7a_bonfil <- read_html(url$url[i])
...
# Guardar en excel
rio::export(ref_tb_viewrec,"ref_tb_viewrec_x7a.xlsx")
